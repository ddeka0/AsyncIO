...
static int recv_prep(struct io_uring *ring, struct iovec *iov)
{
	...
	if (ret <= 0) {
        ...
        std::cout <<"Receive Task submitted" << std::endl;
    }
...
static int do_recv(struct io_uring *ring, struct iovec *iov)
{
	...
	if (cqe->res -1 != strlen(str)) {
		...
		std::cout <<"cqe->user_data : "<< cqe->user_data << std::endl;
		std::cout <<"cqe->res "<< cqe->res << std::endl;
	}

	if (strcmp(str, (const char *)iov->iov_base)) {
...
static int do_send(void)
{
	...
	if (ret <= 0) {
		...
		std::cout <<"io_uring_submit ret :" << ret << std::endl;
	}
	...
	if (cqe->res == -EINVAL) {
		...
		std::cout <<"io_uring_wait_cqe ret :" << ret << std::endl;
	}
